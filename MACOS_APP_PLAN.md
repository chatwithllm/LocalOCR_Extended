...
├── LocalOCR/                                   # Main app target (com.localocr.extended)
│   │
...
│   │   │
│   │   ├── KeychainStore.swift                 # SecItemAdd/SecItemCopyMatching wrapper using KeychainAccess SPM; service "com.localocr.extended"; stores: sessionToken (if bearer), oauthRefreshToken; NEVER UserDefaults for credentials
...
│   │   └── QuickLookExtension/                 # [v1.1] Separate App Extension target (com.localocr.extended.quicklook); QLPreviewingController for locally-cached receipt images
...
│
├── LocalOCRTests/                              # Unit test target (com.localocr.extendedTests)
...
│
├── LocalOCRUITests/                            # XCUITest E2E target (com.localocr.extendedUITests)
...
| **OAuth refresh token** | Google OAuth refresh token (if Google OAuth login used) | Keychain via KeychainAccess; service `"com.localocr.extended"`, key `"oauth.refreshToken"` | None | Set by `AuthState.handleGoogleOAuthCallback()` after the server returns the OAuth session. Used to re-authenticate silently on session expiry. |
...
- Cache location: `~/Library/Caches/com.localocr.extended/<endpoint-hash>.json`
...
- Queue location: `~/Library/Application Support/com.localocr.extended/pendingWrites.sqlite`
...
| Service identifier | `"com.localocr.extended"` |
...
|---|---|
| URL scheme | `localocr://` |
| Registration | `Info.plist` key `CFBundleURLTypes`: array with one entry `{CFBundleURLName: "com.localocr.extended.url", CFBundleURLSchemes: ["localocr"]}` |
...
| Target name | `LocalOCRQuickLook` (bundle ID `com.localocr.extended.quicklook`) |
...
**Debug tools**:
- `os.log` framework for structured logging (`Logger(subsystem: "com.localocr.extended", category: "networking")`). Visible in Console.app filtered by subsystem.
...
- [ ] Create Xcode project `LocalOCR.macOS`, bundle ID `com.localocr.extended`, deployment target macOS 13.0, Swift 5.10 language version
...
|---|---|---|---|---|
| `LocalOCRTests` | `com.localocr.extendedTests` | Unit | `LocalOCR.app` | `ComponentTests/`, `ViewTests/`, `NetworkingTests/`, `StateTests/`, `HelpersTests/` |
| `LocalOCRUITests` | `com.localocr.extendedUITests` | XCUITest | `LocalOCR.app` (live, not in-process) | E2E test scripts from §6.4; macOS-specific scenarios from §6.5 |
