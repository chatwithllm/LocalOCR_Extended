...
Recommended encryption mechanism: `cryptography.fernet`

- add a single `FERNET_SECRET_KEY` environment variable to the deployment config
- use it to construct a `Fernet` instance at app startup
- encrypt on admin write, decrypt in the router immediately before client instantiation
- never log decrypted values or include them in error responses

```python
from cryptography.fernet import Fernet

def encrypt_api_key(plaintext: str) -> str:
    f = Fernet(current_app.config["FERNET_SECRET_KEY"])
...
Without pinning this down, two developers could implement key storage differently and produce incompatible ciphertext in the database.
...
All four capability flags from `ai_model_configs` should be included in this response. The frontend needs them to filter models by the current file type being processed and to render accurate capability badges. This matters especially for OpenRouter entries, where capability varies significantly per upstream model.
...
The router should return a structured error result rather than raising unhandled exceptions into the receipt flow.
...
{
    "error": True,
    "error_code": "provider_auth_failed",  # rate_limit | timeout | unsupported_file_type | provider_error
    "provider": "openrouter",
    "model_string": "google/gemini-2.0-flash",
    "message": "...",
    "retryable": False,
...
Recommended behavior per error type:

- `provider_auth_failed` — surface immediately, do not retry, prompt admin to check credentials
- `rate_limit` — retry once with exponential backoff, then surface error to caller
- `timeout` — retry once, then surface error; log latency for observability
- `unsupported_file_type` — fail fast before dispatch, return immediately without hitting the provider

The receipt upload and reprocess handlers should check for `result.get("error")` and return an appropriate HTTP error response rather than attempting to parse failed output as receipt data.
...
### Handling disabled models

If an admin disables a model that users have set as their active selection, those users will otherwise fail on their next upload or reprocess request.

Recommended behavior when the resolved active model is disabled:

- fall back to the environment default provider
- clear `active_ai_model_config_id` on the user record so the stale selection does not persist
- include a warning in the response so the frontend can notify the user
- log the fallback event server-side for observability and support debugging

```python
if resolved_model and not resolved_model.is_enabled:
    disabled_model_id = resolved_model.id
    current_user.active_ai_model_config_id = None
    db.session.commit()
    resolved_model = None  # triggers env fallback in router
    response_warnings.append("Your selected model has been disabled. Falling back to default.")
    logger.warning(
        "Disabled model fallback applied",
        extra={"user_id": current_user.id, "model_config_id": disabled_model_id},
    )
```

This keeps the app functional without requiring the user to manually re-select a model.
...
If time is tight, Anthropic can be scaffolded in Phase 1 without enabling any Anthropic model config yet. That still keeps the router structure complete and avoids a later structural gap when Anthropic is turned on.
