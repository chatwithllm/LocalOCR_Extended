...
The router must return a structured error result rather than raising unhandled exceptions into the receipt flow.
...
If an admin disables a model that users have set as their active selection, those users will hit errors silently on their next upload or reprocess request.

To prevent this, the upload and reprocess handlers should verify `is_enabled` on the resolved model at request time, not just at selection time.

Recommended behavior when the active model is found to be disabled:
...
- implement unified provider routing for:
  ...
  - Ollama
  - OpenRouter
  - Anthropic (scaffold the branch and provider function in Phase 1 even if no Anthropic model config is enabled yet — this keeps the router complete and avoids a structural gap when Anthropic is enabled later)
