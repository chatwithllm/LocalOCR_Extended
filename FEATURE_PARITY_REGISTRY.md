...
| Row ID | Screen | UI Element | Action / Verb | Endpoint | Web Impl Notes | Android Impl | Status |
|--------|--------|-----------|---------------|----------|----------------|--------------|--------|
| F-601 | Restaurant | Stat — Visits count | display | GET `/analytics/restaurant-summary?months=N` | RULE 1: web reads `summaryData.visit_count` at index.html:35500; primary endpoint is `/analytics/restaurant-summary`, NOT `/receipts?type=restaurant` (which is the "Open All" deep-link, not the count source) | `_StatCard(key:'restaurant-visit-count')` value=`summary.visitCount`; sub shows refund count or 'restaurant purchases' | ✅ |
| F-602 | Restaurant | Stat — Dining Spend | display | GET `/analytics/restaurant-summary` | RULE 1: web prefers `summaryData.total_spend` (index.html:35506); falls back to `/analytics/spending?period=monthly&domain=restaurant&months=N` `grand_total`. Android reads `summary.totalSpend` from the same restaurant-summary call. | `_StatCard(key:'restaurant-total-spend')` value=`_money.format(summary.totalSpend)` | ✅ |
| F-603 | Restaurant | Stat — Average Ticket | display | GET `/analytics/restaurant-summary` | `summaryData.average_ticket` (index.html:35511) — computed backend-side as `purchase_total / purchase_count` (calculate_spending_analytics.py:458) | `_StatCard(key:'restaurant-average-ticket')` value=`_money.format(summary.averageTicket)` | ✅ |
| F-604 | Restaurant | Stat — Top Restaurant | display | GET `/analytics/restaurant-summary` | RULE 1: web reads `(summaryData.top_restaurants \|\| [])[0]` (index.html:35483/35513); no separate `/analytics/top-merchants` endpoint is hit. Backend exposes `top_restaurants` array inside restaurant-summary body. | `_StatCard(key:'restaurant-top-store')` value=`topStore.store`; sub shows visits / refunds / Net total | ✅ |
| F-605 | Restaurant | Dining Budget — month picker | month-input | GET `/budget/status?month=YYYY-MM&domain=restaurant` | `loadRestaurantBudget()` (V-9 RESOLVED — endpoint corrected from fabricated `/budget/dining`; real endpoint per `manage_household_budget.py:284`) | `_BudgetCard` `TextButton.icon(key:'restaurant-budget-month')` → `showDatePicker` → writes `restaurantBudgetMonthProvider` (YYYY-MM); cabinet provider re-fires on change | ✅ |
| F-606 | Restaurant | Dining Budget — amount input | number-input | POST `/budget/set-monthly` | RULE 1: web posts to `/budget/set-monthly` body `{month, domain:'restaurant', budget_amount}` (index.html:35643). Registry previously cited `/budget category=dining` — real endpoint per `manage_household_budget.py:160`. | `_BudgetCard` `TextField(key:'restaurant-budget-amount', keyboardType: decimal)` controller `_amount` (admin-only) | ✅ |
| F-607 | Restaurant | Dining Budget — Save button | button | POST `/budget/set-monthly` | `saveRestaurantBudget()` (corrected endpoint per F-606) | `_BudgetCard` `FilledButton('Save')` → `RestaurantRepository.setBudget` → POST /budget/set-monthly; bundle invalidate + SnackBar | ✅ |
| F-608 | Restaurant | Dining Budget status (progress bar) | display | GET `/budget/status?month=&domain=restaurant` | `restaurant-budget-status` (V-9 RESOLVED) | `_BudgetCard` `LinearProgressIndicator(value=pct/100)` color cycles green (<70) / amber (70-90) / error (>=90); meta line `{pct}% used · N visits · M refunds` + remaining/over label | ✅ |
| F-609 | Restaurant | Receipt Review period select | select | GET `/analytics/restaurant-summary?months=N` | RULE 1: 3/6/12 months — web wires `restaurant-period` select to `loadRestaurant()` which feeds `months` query into BOTH `/analytics/restaurant-summary` AND `/analytics/spending` (index.html:35469-35473). Registry previously cited `/receipts?type=restaurant&months=N` — that endpoint exists for "Open All" deep-link but is not what the stats use. | `_StatsGrid` `DropdownButton<int>(key:'restaurant-period')` w/ `restaurantPeriodOptions` (3/6/12) bound to `restaurantPeriodProvider` | ✅ |
| F-610 | Restaurant | Receipt Review refresh 🔄 button | button | GET `/analytics/restaurant-summary` | `loadRestaurant()` (corrected endpoint per F-609) | AppBar `IconButton(Icons.refresh)` → `ref.invalidate(restaurantBundleProvider)` | ✅ |
| F-611 | Restaurant | Restaurant body — receipt row tap | tap | GET `/receipts/<id>` | inline detail | `_ReceiptsCard` `ListTile.onTap` → `GoRouter.go('/receipts/${purchaseId}')`; Receipt detail screen is currently a PlaceholderScreen — deep-link itself works and routes through ShellRoute (Receipts wave will hydrate the detail body) | ✅ |
| F-612 | Restaurant | Top Restaurants list row tap | tap | GET `/receipts?store=...` | filter receipts to store | `_TopRestaurantsCard` `ListTile.onTap` → `GoRouter.go('/receipts?store=${encodeQueryComponent(store)}')`; same Receipts placeholder caveat as F-611 | ✅ |
| F-613 | Restaurant | Top Ordered Items row | display | — | RULE 1: web `topItems` block is a static table — no `onclick` on row, no `/receipts?item=...` filter exists at index.html:35559 (the registry "tap" verb was incorrect). Web renders qty + total + avg columns only. | `_TopItemsCard` per-item `Row(Expanded name, ×qty, total, avg)`; display only — matches web exactly (no tap action because web has none) | ✅ |
---
